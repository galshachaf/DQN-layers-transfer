...
stats_evl_smarteps_vals[1] = {["use_smart_eps"]=use_smart_eps_default_value,["smart_eps_choice_type"]=smart_eps_choice_type_default_value,["safeeps_nloest"]=safe_eps_n_lowest_default_value,["epsilon_val"]=opt.EVAL_EPS}
stats_evl_smarteps_vals[2] = {["use_smart_eps"]=false,                      ["smart_eps_choice_type"]="safeeps_nloest",                   ["safeeps_nloest"]=0,                              ["epsilon_val"]=0.05}
stats_evl_smarteps_vals[3] = {["use_smart_eps"]=true,                       ["smart_eps_choice_type"]="safeeps_nloest",                   ["safeeps_nloest"]=3,                              ["epsilon_val"]=0.0}
...
for rst_idx=1,max_level_stats do
        if (si_split_agent and rst_idx > 1) or (learn_level[rst_idx] and learn_level[rst_idx] < 1) then
                learning_active_for_level[rst_idx] = false
                notlearning_epsilon_for_level[rst_idx] = learn_level[rst_idx]
        else
...
----------------------------------------------------
-- Screen analysis functions (mainly for terminal counter)
----------------------------------------------------

function screen_get_pixel_count(screen,coordinates,color_ind,color_val)
	if color_ind then
		if color_val then
			return screen[1][color_ind][coordinates][screen[1][color_ind][coordinates]:eq(color_val)]:nElement()
		else
			return screen[1][color_ind][coordinates][screen[1][color_ind][coordinates]:gt(0)]:nElement()
		end
	end
	pix_count = 0
	for color_ind=1,3 do
		if color_val then
			pix_count = pix_count + screen[1][color_ind][coordinates][screen[1][color_ind][coordinates]:eq(color_val)]:nElement()
		else
			pix_count = pix_count + screen[1][color_ind][coordinates][screen[1][color_ind][coordinates]:gt(0)]:nElement()
		end
	end
	return pix_count
end
...
function agent_perceive(reward, screen, terminal, param1, param2, agent_to_use)
        recursive_call = false
        if agent_to_use then
                recursive_call = true
        end
        agent_to_use = agent_to_use or get_agent_id(current_level)
        if not (agent[agent_to_use]) then
                agent_to_use = 1
                param1 = true
                param2 = param2 or 0.05
        end
        selected_act = 0
        if param1 and (param2 ~= nil) then -- Verify 0.0 ~= nil
                selected_act = agent[agent_to_use]:perceive(reward, screen, terminal, param1, param2)
        elseif (not recursive_call) and learning_active_for_level[current_level] and learning_active_for_agent[agent_to_use] then
                -- Update learning stats
                stats_agent_learning_steps_level[agent_to_use][current_level] = (stats_agent_learning_steps_level[agent_to_use][current_level] or 0) + 1
                -- Learn
                selected_act = agent[agent_to_use]:perceive(reward, screen, terminal)
        else
                -- Else: Dont learn
                selected_act = agent[agent_to_use]:perceive(reward, screen, terminal, true, notlearning_epsilon_for_level[current_level])
                if do_debug_prints then
                        print("Not learning level ",current_level," and using epsilon ",notlearning_epsilon_for_level[current_level])
                end
        end

        if selected_act > skillplay_max_original_action then
                -- We decided to use skills.!
                -- Our skill is using an agent of a different level.. (use it without epsilon greedy and don't let it learn)

                -- Translate action to agent
                selected_agent_to_use = (selected_act - skillplay_max_original_action)

                -- Update stats
                stats_ttl_choosed_skill[agent_to_use] = (stats_ttl_choosed_skill[agent_to_use] or 0) + 1
                stats_ttl_choosed_skill_detailed[agent_to_use][selected_agent_to_use] = (stats_ttl_choosed_skill_detailed[agent_to_use][selected_agent_to_use] or 0) + 1
                stats_ttl_choosed_skill_per_level[agent_to_use][current_level] = (stats_ttl_choosed_skill_per_level[agent_to_use][current_level] or 0) + 1
                stats_evl_choosed_skill[agent_to_use] = (stats_evl_choosed_skill[agent_to_use] or 0) + 1
                stats_evl_choosed_skill_detailed[agent_to_use][selected_agent_to_use] = (stats_evl_choosed_skill_detailed[agent_to_use][selected_agent_to_use] or 0) + 1
                stats_evl_choosed_skill_per_level[agent_to_use][current_level] = (stats_evl_choosed_skill_per_level[agent_to_use][current_level] or 0) + 1

                -- Get action from the chosen agent (the "skill")
                selected_act = agent_perceive(reward, screen, terminal, true, 0.0, selected_agent_to_use)
        end
        return selected_act
end
...
function get_string_of_var(var_to_str)
      ...
      if type(var_to_str) == type(bool) then
        if var_to_str then
          return "true"
        else
          return "false"
...
while step < opt.steps do
        ...
        if (step % opt.eval_freq == 0 and step > learn_start) or (eval_only_mode) then
          ...
          for smarteps_idx in pairs(stats_evl_smarteps_vals) do
            ...
            local cur_steps = {}
            local cur_ag = {}
            for level = 1,max_level_stats do
                    cur_ep[level] = (agent[get_agent_id(level)].ep_end +
                    math.max(0, (agent[get_agent_id(level)].ep_start - agent[get_agent_id(level)].ep_end) * (agent[get_agent_id(level)].ep_endt -
                    math.max(0, agent[get_agent_id(level)].numSteps - agent[get_agent_id(level)].learn_start))/agent[get_agent_id(level)].ep_endt))
                    cur_steps[level] = agent[get_agent_id(level)].numSteps
                    cur_ag[level] = get_agent_id(level)
            end
            ...
            print(string.format(
                ...
                'current_eval_eps: %s , agents_use_smart_eps_string_vals: %s , agents_safe_eps_nlowest_vals: %s , agents_smart_eps_choice_type_vals: %s ,' ..
                ...
                'stats_ttl_choosed_skill: %s , stats_ttl_choosed_skill_detailed: %s , stats_ttl_choosed_skill_per_level: %s , ' ..
                'stats_evl_choosed_skill: %s , stats_evl_choosed_skill_detailed: %s , stats_evl_choosed_skill_per_level: %s ,' ..
                ...
                step, step*opt.actrep, total_reward, agent[get_agent_id(current_level)].ep, agent[get_agent_id(current_level)].lr, time_dif,
                training_rate, eval_time, opt.actrep*opt.eval_steps/eval_time,
                nepisodes, nrewards,
                get_string_of_vals(cur_ag,"cur_ag"), get_string_of_vals(cur_ep,"cur_ep"), get_string_of_vals(cur_steps,"cur_steps"),
                current_eval_eps,get_string_of_vals(agents_use_smart_eps_string_vals,"agents_use_smart_eps_string_vals"),get_string_of_vals(agents_safe_eps_nlowest_vals,"agents_safe_eps_nlowest_vals"),get_string_of_vals(agents_smart_eps_choice_type_vals,"agents_smart_eps_choice_type_vals"),
                ...
                get_string_of_vals(stats_ttl_choosed_skill,"stats_ttl_choosed_skill"), get_string_of_vals(stats_ttl_choosed_skill_detailed,"stats_ttl_choosed_skill_detailed"), get_string_of_vals(stats_ttl_choosed_skill_per_level,"stats_ttl_choosed_skill_per_level"),
                get_string_of_vals(stats_evl_choosed_skill,"stats_evl_choosed_skill"), get_string_of_vals(stats_evl_choosed_skill_detailed,"stats_evl_choosed_skill_detailed"), get_string_of_vals(stats_evl_choosed_skill_per_level,"stats_evl_choosed_skill_per_level"),
